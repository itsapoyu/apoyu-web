...
- **RENDERER FIDELITY and SPECIMEN VALIDITY are separate questions.** The renderer is a port
  (above). The SPECIMEN is what it is asked to draw. The default hero draws a PRODUCTION-VALID
  mint: rarity `rare`, because `archetypes.ts` pins this archetype to `minRarity = maxRarity =
  'rare'` and the engine clamps every roll to that; comfort `dirt`, because
  `assignComfortType(38)` returns it and `zoneOfComfort('dirt')` is the red zone this archetype
  serves; and a `why` GENERATED by the real `presentReceiptWhy()`, not the visual-lab line
  (which that module explicitly says is not production truth). `#legendary-stress` swaps in the
  lab fixture, which exercises the full finish vocabulary but is **not a mintable card** and must
  never be used as the side-by-side fidelity reference.
